...
export const useSpeechStore = create<SpeechStore>((set) => ({
  ...
  addTranscript: (t) =>
    set((state) => {
      ...
      return {
        ...
        recentBuffer: [t, ...state.recentBuffer].filter(
          (x) => now - x.timestamp.getTime() < BUFFER_WINDOW_MS
        ),
      };
    }),

  addAnalysis: (a) =>
    set((state) => ({ analyses: [a, ...state.analyses] })),

  addDetectedQuestion: (q) =>
    set((state) => ({
      detectedQuestions: [q, ...state.detectedQuestions].slice(0, 20),
    })),

  markQuestionAnalyzed: (id) =>
    set((state) => ({
      detectedQuestions: state.detectedQuestions.map((q) =>
        q.id === id ? { ...q, analyzed: true } : q
      ),
    })),
  ...
  clearBuffer: () => set({ recentBuffer: [], selectedChunkIds: new Set() }),
  clear: () =>
    set({
      transcripts: [],
      recentBuffer: [],
